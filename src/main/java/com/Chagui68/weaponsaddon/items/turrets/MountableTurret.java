...
public class MountableTurret extends CustomRecipeItem implements EnergyNetComponent, Listener {
    ...
    public static final SlimefunItemStack MOUNTABLE_TURRET = new SlimefunItemStack("MA_MOUNTABLE_TURRET", Material.OBSERVER, "&4💣 &cWraith-Class War Turret", "", "&7A heavy player-controlled plasma turret.", "&7Advanced energy-based war machine.", "", "&6Target: &eAny (Including Players)", "&6Damage: &e10.0 HP", "&6Range: &e35 Blocks", "&6Energy: &e150 J per shot", "", "&eRight-Click the base to mount", "&eLeft-Click while mounted to fire");
    ...
    private void tick(Block b) {
        ...
        Entity modelPart = loc.getWorld().getNearbyEntities(loc.clone().add(0.5, 0.5, 0.5), 1.5, 1.5, 1.5).stream().filter(e -> e.getScoreboardTags().contains(tag)).findFirst().orElse(null);
        if (modelPart == null) {
            spawnModel(loc);
        }
    ...
    private void spawnModel(Location loc) {
        ...
        base.setTransformation(new Transformation(new Vector3f(-0.5f, 0.15f, -0.5f), new Quaternionf(), new Vector3f(1.0f, 0.25f, 1.0f), new Quaternionf()));
        ...
        for (float[] offset : legOffsets) {
            ...
            leg.setTransformation(new Transformation(new Vector3f(offset[0] - 0.2f, 0.0f, offset[1] - 0.2f), new Quaternionf(), new Vector3f(0.4f, 0.3f, 0.4f), new Quaternionf()));
        ...
        glow.setTransformation(new Transformation(new Vector3f(-0.3f, 0.18f, -0.3f), new Quaternionf(), new Vector3f(0.6f, 0.05f, 0.6f), new Quaternionf()));
        ...
        head.setTransformation(new Transformation(new Vector3f(-0.4f, -0.3f, -0.4f), new Quaternionf(), new Vector3f(0.8f, 0.7f, 0.8f), new Quaternionf()));
        ...
        for (float x : sideOffsets) {
            ...
            b.setTransformation(new Transformation(new Vector3f(x + 0.05f, 0.0f, 0.4f), new Quaternionf(), new Vector3f(0.15f, 0.15f, 1.4f), new Quaternionf()));
            ...
            w.setTransformation(new Transformation(new Vector3f(x - 0.1f, -0.3f, 0.0f), new Quaternionf(), new Vector3f(0.4f, 1.0f, 0.6f), new Quaternionf()));
            ...
            wg.setTransformation(new Transformation(new Vector3f(x - 0.11f, 0.1f, 0.2f), new Quaternionf(), new Vector3f(0.42f, 0.1f, 0.2f), new Quaternionf()));
        ...
        ring.setTransformation(new Transformation(new Vector3f(-0.55f, -0.7f, -0.65f), new Quaternionf(), new Vector3f(1.1f, 1.5f, 0.2f), new Quaternionf()));
    ...
    public void onHitboxInteract(PlayerInteractEntityEvent e) {
        ...
        if (!interaction.getScoreboardTags().contains("MOUNT_HITBOX")) return;
    ...
    public void onPlayerInteract(PlayerInteractAtEntityEvent e) {
        ...
        Interaction interaction = (Interaction) e.getRightClicked();
        if (!interaction.getScoreboardTags().contains("MOUNT_HITBOX")) return;
        String tag = interaction.getScoreboardTags().stream().filter(t -> t.startsWith("MOUNT_TURRET_")).findFirst().orElse(null);
        if (tag == null) return;
        ArmorStand seat = (ArmorStand) interaction.getWorld().getNearbyEntities(interaction.getLocation(), 1.5, 1.5, 1.5).stream().filter(ent -> ent.getScoreboardTags().contains(tag) && ent.getScoreboardTags().contains("MOUNT_SEAT")).findFirst().orElse(null);
        if (seat != null) {
    ...
    private boolean isMounting(Player player, Interaction interaction) {
        if (player.getVehicle() instanceof ArmorStand && player.getVehicle().getScoreboardTags().contains("MOUNT_SEAT")) {
            String turretTag = interaction.getScoreboardTags().stream().filter(t -> t.startsWith("MOUNT_TURRET_")).findFirst().orElse(null);
    ...
    private void shoot(Player shooter, Interaction interaction) {
        ...
        if (tag == null) return;
        String[] parts = tag.split("_");
        Location loc = new Location(interaction.getWorld(), Integer.parseInt(parts[2]), Integer.parseInt(parts[3]), Integer.parseInt(parts[4]));
        ...
        world.spawnParticle(Particle.SOUL_FIRE_FLAME, start.clone().add(direction.clone().multiply(1.5)), 20, 0.2, 0.2, 0.2, 0.1);
        ...
        double maxDist = (blockResult != null && blockResult.getHitBlock() != null) ? blockResult.getHitBlock().getLocation().distance(start) : RANGE;
        RayTraceResult entityResult = world.rayTraceEntities(start, direction, maxDist, 0.5, ent -> ent instanceof LivingEntity && ent != shooter && !(ent instanceof ArmorStand) && !(ent instanceof Interaction));
        ...
        for (int i = 0; i < 25; i++) {
            ...
            if (bullet.distanceSquared(start) > maxDist * maxDist) break;
    ...
    private void updateModelRotation(Location loc) {
        String tag = "MOUNT_TURRET_" + loc.getBlockX() + "_" + loc.getBlockY() + "_" + loc.getBlockZ();
        ArmorStand seat = (ArmorStand) loc.getWorld().getNearbyEntities(loc.clone().add(0.5, 0.5, 0.5), 1.5, 1.5, 1.5).stream().filter(e -> e.getScoreboardTags().contains(tag) && e.getScoreboardTags().contains("MOUNT_SEAT")).findFirst().orElse(null);
        if (seat == null || seat.getPassengers().isEmpty()) return;
        if (!(seat.getPassengers().get(0) instanceof Player)) return;
        ...
        for (Entity entity : loc.getWorld().getNearbyEntities(loc.clone().add(0.5, 0.5, 0.5), 1.5, 1.5, 1.5)) {
            if (entity.getScoreboardTags().contains(tag) && (entity.getScoreboardTags().contains("MOUNT_HEAD") || entity.getScoreboardTags().contains("MOUNT_BARREL"))) {
