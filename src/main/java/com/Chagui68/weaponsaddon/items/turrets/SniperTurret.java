...
public class SniperTurret extends CustomRecipeItem implements EnergyNetComponent, Listener {
    ...
    public static final SlimefunItemStack SNIPER_TURRET = new SlimefunItemStack("MA_SNIPER_TURRET", Material.RED_STAINED_GLASS, "&1🎯 &9Long-Range Sniper Turret", "", "&7Precision-engineered for long-range", "&7elimination of hostile targets.", "", "&6Maximum Range: &e45 Blocks", "&6Precision Damage: &e30.0 HP", "&6Fire Rate: &eVery Slow (3s)", "&6Energy: &e250 J per shot", "&6Capacity: &e6000 J", "", "&eRight-Click to place", "&8(Animated 3D Model)");
    ...
    private void shoot(Location start, LivingEntity target) {
        ...
        Location muzzle = start.clone().add(-Math.sin(radYaw) * (BARREL_SCALE_Z + 0.1), 0.4 + (HEAD_OFFSET_Y - 0.95), Math.cos(radYaw) * (BARREL_SCALE_Z + 0.1));
    ...
    private void spawnPvzModel(Location loc) {
        ...
        stem.setTransformation(new Transformation(new Vector3f(STEM_OFFSET_X, 0.0f, STEM_OFFSET_X), new Quaternionf(), new Vector3f(STEM_SCALE_X, STEM_SCALE_Y, STEM_SCALE_Z), new Quaternionf()));
        ...
        frame.setTransformation(new Transformation(new Vector3f(-0.2f, 0.0f, -0.2f), new Quaternionf(), new Vector3f(0.4f, STEM_SCALE_Y * 0.9f, 0.4f), new Quaternionf()));
        ...
        head.setTransformation(new Transformation(new Vector3f(HEAD_INT_OFFSET, -0.15f, HEAD_INT_OFFSET), new Quaternionf(), new Vector3f(HEAD_SCALE, HEAD_SCALE, HEAD_SCALE), new Quaternionf()));
        ...
        mouth.setTransformation(new Transformation(new Vector3f(-BARREL_SCALE_X / 2f, 0.0f, BARREL_OFFSET_Z), new Quaternionf(), new Vector3f(BARREL_SCALE_X, BARREL_SCALE_Y, BARREL_SCALE_Z), new Quaternionf()));
        ...
        sensor.setTransformation(new Transformation(new Vector3f(SENSOR_OFFSET_X, SENSOR_OFFSET_Y, SENSOR_OFFSET_Z), new Quaternionf(), new Vector3f(SENSOR_SCALE, SENSOR_SCALE, SENSOR_SCALE), new Quaternionf()));
    ...
    public void onHitboxAttack(EntityDamageByEntityEvent e) {
        ...
        if (!interaction.getScoreboardTags().contains("SNIPER_HITBOX")) return;
            ...
            if (tag.startsWith("PVZ_SNIPER_")) {
                ...
                if (parts.length == 5) {
                    try {
                        ...
                        if (id != null && id.equals("MA_SNIPER_TURRET")) {
    ...
    private void updateModelRotation(Location loc, LivingEntity target) {
        ...
        for (Entity entity : loc.getWorld().getNearbyEntities(center, 1.5, 1.5, 1.5)) {
            if (entity.getScoreboardTags().contains(tag) && (entity.getScoreboardTags().contains("PVZ_HEAD") || entity.getScoreboardTags().contains("PVZ_MOUTH") || entity.getScoreboardTags().contains("PVZ_SENSOR"))) {
    ...
    public static void register(SlimefunAddon addon, ItemGroup category) {
        ItemStack[] recipe = new ItemStack[]{MilitaryComponents.FIREARM_BARREL, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.FIREARM_BARREL, MilitaryComponents.KINETIC_STABILIZER, MilitaryComponents.TARGETING_SYSTEM, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.TARGETING_SYSTEM, MilitaryComponents.KINETIC_STABILIZER, MilitaryComponents.MOVEMENT_CIRCUIT, MilitaryComponents.TUNGSTEN_INGOT, MilitaryVouchers.VOUCHER_DOCUMENT, MilitaryComponents.TURRET_SHELL, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.MOVEMENT_CIRCUIT, MilitaryComponents.MOVEMENT_CIRCUIT, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.TURRET_SHELL, MilitaryVouchers.VOUCHER_WEAPON_UPGRADE, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.MOVEMENT_CIRCUIT, MilitaryComponents.KINETIC_STABILIZER, MilitaryComponents.TARGETING_SYSTEM, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.TUNGSTEN_INGOT, MilitaryComponents.TARGETING_SYSTEM, MilitaryComponents.KINETIC_STABILIZER, MilitaryComponents.FIREARM_BARREL, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.ENERGY_MATRIX, MilitaryComponents.FIREARM_BARREL};
